...
function ObracunTab({ st, showToast, voznikId }) {
  ...
  const [saving,setSaving]=useState(false);const [saved,setSaved]=useState(false);const [existingId,setExistingId]=useState(null);const [zadnjeShranjeno,setZadnjeShranjeno]=useState(null);
  ...
  return(<div>
    <div style={s.card}><div style={s.cardTitle}>📅 Tedenski obračun</div><div style={s.datumRow}><div style={s.datumPolje}><label style={s.smallLabel}>Od</label><input style={s.input} type="date" value={datZac} onChange={e=>{const pon=getMonday(e.target.value);const ponStr=pon.toISOString().slice(0,10);setDatZac(ponStr);const d=new Date(pon);d.setDate(d.getDate()+6);setDatKon(d.toISOString().slice(0,10));}}/></div><div style={s.datumSep}>–</div><div style={s.datumPolje}><label style={s.smallLabel}>Do</label><input style={s.input} type="date" value={datKon} onChange={e=>setDatKon(e.target.value)}/></div></div><div style={s.obdobjeLabel}>{obdobjeLabel}</div></div>
    {!saved&&<div style={{...s.card,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}><div><div style={{fontSize:13,fontWeight:700,color:"#0f2744"}}>💾 Sprotno shranjevanje</div><div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>{zadnjeShranjeno?`Nazadnje shranjeno ob ${zadnjeShranjeno}`:"Vpisuj med tednom in shrani — podatki ostanejo."}</div></div><button style={{background:"#0f2744",color:"#fff",border:"none",borderRadius:10,padding:"10px 16px",fontSize:13,fontWeight:700,cursor:"pointer",opacity:saving?0.5:1,flexShrink:0}} onClick={()=>shrani("osnutek")} disabled={saving}>{saving?"⏳":"💾 Shrani"}</button></div>}
...
const s={
  ...
  input:{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:10,padding:"11px 13px",fontSize:15,outline:"none",boxSizing:"border-box",background:"#f8fafc",color:"#0f2744",WebkitTextFillColor:"#0f2744",colorScheme:"light"},
