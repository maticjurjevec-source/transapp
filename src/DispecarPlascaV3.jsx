...
function GpsTab({nalogi,vozniki}){
  ...
  return (<div>
    ...
    {list.map(v=>(<div key={v.id} onClick={()=>setOdpV(p=>({...p,[v.id]:!p[v.id]}))} style={{cursor:"pointer",textAlign:"left",background:"#fff",borderRadius:12,padding:14,marginBottom:10,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",borderLeft:v.hitrost>0?"4px solid #22c55e":"4px solid #cbd5e1"}}>
      ...
      <div style={{fontSize:13,color:"#64748b",marginBottom:6}}>{v.lokacija||"Lokacija ni znana"}</div>{(()=>{const nr=(x)=>(x||"").toUpperCase().replace(/[\s.-]/g,"");const vo=(vozniki||[]).find(x=>nr(x.vozilo)===nr(v.reg_tablica));if(!vo)return null;const ms=(x)=>x?new Date(x+"T00:00:00").getTime():0;const akt=(nalogi||[]).filter(x=>x.voznikId===vo.id&&x.status!=="zakljucen"&&x.status!=="fakturirano"&&x.status!=="za_fakturo").sort((a,b)=>ms(a.nakDatum)-ms(b.nakDatum));if(!akt.length)return null;const odp=!!odpV[v.id];const now=Date.now();const g=(c)=>"https://www.google.com/maps/dir/?api=1&origin="+v.lat+","+v.lon+"&destination="+encodeURIComponent(c)+"&travelmode=driving";const bs={fontSize:12,color:"#0f2744",background:"#fff",border:"1px solid #e2e8f0",padding:"5px 12px",borderRadius:8,fontWeight:700,textDecoration:"none",display:"inline-block"};return <div style={{marginBottom:8}}><div style={{fontSize:12,color:"#2563eb",fontWeight:700,marginBottom:odp?8:0}}>{akt.length+(akt.length===1?" aktiven nalog ":" aktivni nalogi ")+(odp?"\u25b2":"\u25bc")}</div>{odp&&akt.map(n=>{const teku=ms(n.nakDatum)<=now&&ms(n.razDatum)+86400000>=now;return <div key={n.id} style={{background:"#f8fafc",borderRadius:8,padding:"8px 10px",marginBottom:6,borderLeft:teku?"3px solid #2563eb":"3px solid #e2e8f0"}}><div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:3}}><span style={{fontSize:11,fontFamily:"monospace",color:"#2563eb",fontWeight:700}}>{n.stevilkaNaloga}</span>{teku&&<span style={{fontSize:10,background:"#dbeafe",color:"#1d4ed8",padding:"1px 7px",borderRadius:20,fontWeight:700}}>v teku</span>}<span style={{fontSize:11,color:"#94a3b8"}}>{n.stranka||""}</span></div><div style={{fontSize:12,color:"#334155",fontWeight:600,marginBottom:3}}>{(n.nakKraj||"?")+" \u2192 "+(n.razKraj||"?")}</div><div style={{fontSize:11,color:"#64748b",marginBottom:6}}>{"Nakl.: "+(n.nakDatum||"-")+" "+(n.nakCas||"")+"  |  Razkl.: "+(n.razDatum||"-")+" "+(n.razCas||"")}</div>{v.lat&&<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{(n.nakNaslov||n.nakKraj)&&<a onClick={e=>e.stopPropagation()} href={g(n.nakNaslov||n.nakKraj)} target="_blank" rel="noreferrer" style={bs}>Do naklada</a>}{(n.razNaslov||n.razKraj)&&<a onClick={e=>e.stopPropagation()} href={g(n.razNaslov||n.razKraj)} target="_blank" rel="noreferrer" style={bs}>Do razklada</a>}</div>}</div>;})}</div>;})()}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
